module Jekyll
  class EnvironmentVariablesGenerator < Generator
    def generate(site)
      ...
      site.config['url'] = url if url.to_s.strip.empty? != true
      ...
      if ENV.fetch('JEKYLL_STRICT_MODE',
                   :default_need_not_be_a_string).to_s.strip.downcase == 'true'
...
ENV.fetch('JEKYLL_ENV', :default_need_not_be_a_string).to_s.strip || 'production'
      ...
      exit 1 if site.config['environment'] === 'production'
    end
  end
end
